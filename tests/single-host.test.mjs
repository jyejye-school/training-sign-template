...
test('Apps Script 바깥 탭 파비콘은 자체 경로에서 기본값과 기관 설정을 제공한다', () => {
  const body = backend.slice(backend.indexOf('function doGet'), backend.indexOf('function doPost'));
  assert.match(body, /event && event\.pathInfo/);
  assert.match(body, /pathInfo === 'favicon\.svg'/);
  assert.match(body, /\.setFaviconUrl\(webAppUrl \+ '\/favicon\.svg\?v='/);
  assert.match(body, /ContentService\.createTextOutput\(faviconSvg_\(\)\)/);
  assert.match(body, /readSettings_\(\)\.faviconData/);
  assert.match(body, /function defaultFaviconSvg_\(\)[\s\S]*#2563eb[\s\S]*>서명<\/text>/);
  assert.doesNotMatch(body, /DriveApp|setSharing|ANYONE|imageFileId/);
});
