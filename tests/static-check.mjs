...
expect(/function closeAdminAndLogout\(\)[\s\S]*currentShareToken[\s\S]*replaceAppHash\(shareToken\)[\s\S]*initializePublicApp\(\)/.test(app), '관리자 닫기 뒤 올바른 공유 메인 화면을 다시 불러오지 않습니다.');
...
expect(/event && event\.pathInfo/.test(backend) && /pathInfo === 'favicon\.svg'/.test(backend) && /setFaviconUrl\(webAppUrl \+ '\/favicon\.svg\?v='/.test(backend), 'Apps Script 바깥 브라우저 탭의 파비콘 경로가 누락되었습니다.');
expect(/function faviconOutput_\(\)[\s\S]*ContentService\.createTextOutput\(faviconSvg_\(\)\)[\s\S]*MimeType\.XML/.test(backend), '파비콘을 외부 저장소 없이 자체 SVG로 제공하지 않습니다.');
...
expect(/url\.origin === 'https:\/\/script\.google\.com'/.test(app) && /\\\/macros\\\/s/.test(app), '프런트가 주입된 Apps Script /exec 주소를 제한해 사용하지 않습니다.');
expect(/google\?\.script\?\.url\?\.getLocation/.test(app) && /window\.google\.script\.url\.getLocation\(finish\)/.test(app), 'Apps Script IFRAME 바깥 주소의 공유 해시를 읽지 않습니다.');
expect(/function replaceAppHash\(token\)[\s\S]*window\.google\.script\.history\.replace\(\{\}, \{\}, hash\)[\s\S]*history\.replaceState/.test(app), 'Apps Script 주소와 일반 정적 데모 주소의 해시 변경을 구분하지 않습니다.');
expect(!/frontendUrl:\s*baseUrl|request\.frontendUrl/.test(app + backend), '관리자 설정이나 공유 키 API가 브라우저의 프런트 주소를 전송합니다.');
...
expect(/학교 연수 전자서명 시스템 사용설명서/.test(backend), '다른 학교용 배포 사용설명서가 누락되었습니다.');
expect(!/GitHub|github\.com|Use this template|assets\/config\.js|Deploy from a branch|Pages와 관리자/.test(backend), '시트 사용설명서에 별도 홈페이지 배포 절차가 남아 있습니다.');
const guideSectionOne = backend.slice(backend.indexOf("title: '1. 학교용 사본과 웹앱 만들기'"), backend.indexOf("title: '2. 관리자 첫 설정'"));
expect(guideSectionOne && (guideSectionOne.match(/\n\s*\['\d+'/g) || []).length === 3, '배포 안내서 1번이 3단계로 간소화되지 않았습니다.');
expect(/Google에서 확인하지 않은 앱/.test(guideSectionOne) && /개발자 이메일이 본인 계정/.test(guideSectionOne) && /고급 → 학교 연수 전자서명 시스템으로 이동 → 허용/.test(guideSectionOne), '미검증 앱에서 본인 개발자 계정을 확인하고 승인하는 안내가 누락되었습니다.');
expect(/모르는 이메일/.test(guideSectionOne) && /Google Workspace 정책이 차단하면 중단/.test(guideSectionOne) && /소유자만 처음 한 번 승인/.test(guideSectionOne) && /서명 참여자에게는 이 화면이 나타나지 않습니다/.test(guideSectionOne), '낯선 개발자·조직 차단 대응 또는 최초 승인 범위 안내가 누락되었습니다.');
expect(/새 배포/.test(guideSectionOne) && /웹 앱/.test(guideSectionOne) && /나/.test(guideSectionOne) && /모든 사용자\(로그인 없이 접근 가능\)/.test(guideSectionOne) && /\/exec/.test(guideSectionOne), '한 주소에서 화면과 백엔드를 제공하는 웹앱 배포 안내가 누락되었습니다.');
expect(/title: '2\. 관리자 첫 설정'/.test(backend) && /title: '3\. 연수 운영과 출력'/.test(backend) && /title: '4\. 보안·복구'/.test(backend), 'GitHub 단계 삭제 뒤 사용설명서 번호가 이어지지 않습니다.');
expect(!/학교용 사본과 Google 백엔드 만들기|GitHub Pages 화면 만들기|학교별 전자서명 화면 연결하기/.test(backend), '삭제하기로 한 이전 배포 안내서 제목이 남아 있습니다.');
expect(!/\['4', 'Apps Script 열기'|\['6', '권한 승인'|\['7', '웹앱 주소 복사'|\['1', '배포 템플릿 열기'|\['2', '내 저장소 만들기'/.test(backend), '통합하기로 한 이전 배포 안내 단계가 남아 있습니다.');
expect(/\['2', '미서명 현황 확인'/.test(backend) && /\['6', '원본 삭제'/.test(backend), '연수 운영 안내의 미서명 현황 단계 추가 또는 뒤 단계 번호 조정이 누락되었습니다.');
expect(/setRowHeight\(guideRow, step\[4\] \|\| 58\)/.test(backend), '긴 안내 행의 선택 높이 설정이 없습니다.');
expect(!/jyejye-school/.test(backend), 'Apps Script 코드에 이전 조직명이 남아 있습니다.');
